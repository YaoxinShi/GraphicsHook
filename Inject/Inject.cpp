...
bool unsafeInjectDll(DWORD dwProcessId, PCWSTR pszLibFile)
{
    ...
    __try
    {
        ...
        {
            std::cout << "pszLibFileRemote fail\n";
            __leave;
        }
        ...
        {
            std::cout << "WriteProcessMemory fail\n";
            __leave;
        }
        ...
        if (hThread == NULL)
        {
            std::cout << "CreateRemoteThread fail\n";
            __leave;
        }
        ...
        std::cout << "unsafeInjectDll success\n";
...
int main()
{
    ...
    {
        std::wstring windowName = cmdLine[1];
        std::wstring dll = cmdLine[2];

        WindowInfo window_info;
        if (getWindowInfo(windowName, window_info))
        {
            DWORD pId = NULL;
            DWORD threadId = GetWindowThreadProcessId(window_info.hwnd, &pId);
            bool succeed = startInject(pId, threadId, dll);
            return succeed ? 0 : 1;
        }
        else
        {
            std::cout << "process cannot be found\n";
        }
    }
    else
    {
        std::cout << "Inject.exe [PID] [TID] [DLL FILE]\n";
        std::cout << "Inject.exe [WINDOW NAME] [DLL FILE]\n";
    }
}
